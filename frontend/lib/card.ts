...
// Split layout: left = FOCUS SCORE dominant + stats, right = LOCKED IN art placeholder.
// Purple theme, matches reference design. Pure canvas, returns PNG data URL.
...
const BG = "#0E091C";
const CARD = "#cfc6ff";      // light lavender score card
const INK = "#ffffff";
const ACCENT = "#8b7bff";
const DIM = "#9a8fff";
const LINE = "#3a2f66";
...
export function renderFokusCard(d: FokusCardData): string {
  ...
  ctx.fillText("LOCKED IN", rx + (W - rx) / 2, 60);
  ctx.fillStyle = "rgba(139,123,255,0.5)";
  ctx.font = "600 18px 'Helvetica Neue', Arial, sans-serif";
  ctx.textAlign = "right";
  ctx.fillText("// ART PLACEHOLDER", W - 40, H - 40);
  ctx.textAlign = "left";
